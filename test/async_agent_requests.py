...
async def make_bulk_requests(url, data):
    tasks = [timed_task(make_async_post, url, payload) for payload in data]
    results = await asyncio.gather(*tasks)
    
    outputs = []
    durations = []
    for result, duration in results:
        outputs.append(result.json()["output"])  # ดึงค่าผลลัพธ์จาก make_async_post
        durations.append(duration)       # ดึงเวลาที่ใช้สำหรับแต่ละ task

    # แสดงผลเวลา
    for i, duration in enumerate(durations):
        print(f"Task {i + 1}: {outputs[i]} \n         {duration} seconds")

    total_task_time = sum(durations)
    print(f"Sum of all task durations: {total_task_time:.2f} seconds")
...
print(f"MAX Run time: {end_time1 - start_time1} seconds")
